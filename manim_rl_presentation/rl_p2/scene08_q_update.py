...
class Scene08QUpdateProcess(P2BaseScene):
    ...
    def construct(self):
        ...
        term_1 = Text("old estimate", font_size=TYPOGRAPHY.small_size - 1, color=PALETTE.text_muted).next_to(eq_panel, DOWN, buff=0.18).align_to(eq_panel, LEFT)
        term_2 = Text("observed reward", font_size=TYPOGRAPHY.small_size - 1, color=PALETTE.goal).next_to(term_1, DOWN, aligned_edge=LEFT, buff=0.1)
        term_3 = Text("best future guess", font_size=TYPOGRAPHY.small_size - 1, color=PALETTE.accent).next_to(term_2, DOWN, aligned_edge=LEFT, buff=0.1)
        term_4 = Text("small correction toward target", font_size=TYPOGRAPHY.small_size - 1, color=PALETTE.text_primary).next_to(term_3, DOWN, aligned_edge=LEFT, buff=0.1)
        ...
        trans.next_to(term_4, DOWN, buff=0.22).align_to(eq_panel, LEFT)
