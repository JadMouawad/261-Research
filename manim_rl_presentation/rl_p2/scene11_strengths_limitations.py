...
class Scene11StrengthsLimitations(P2BaseScene):
    ...
    def construct(self):
        ...
        state = Text("State", font_size=TYPOGRAPHY.small_size, color=PALETTE.text_primary).move_to(LEFT * 3.3 + UP * 1.1)
        action = Text("Action", font_size=TYPOGRAPHY.small_size, color=PALETTE.text_primary).move_to(LEFT * 0.9 + UP * 1.1)
        reward = Text("Reward", font_size=TYPOGRAPHY.small_size, color=PALETTE.text_primary).move_to(RIGHT * 1.5 + UP * 1.1)
        next_state = Text("Next state", font_size=TYPOGRAPHY.small_size, color=PALETTE.text_primary).move_to(RIGHT * 4.2 + UP * 1.1)
        ...
        columns = ComparisonColumns("Strengths", strengths, "Limitations", limitations).shift(DOWN * 0.95)
        ...
        recap_group = VGroup(recap_panel, recap_lines).to_edge(DOWN, buff=0.1)
